...
#             label = label[:,j+3:j+27]
        ...
        if len(scores) > 1:
            if score <= scores[-2]:
                print("Model_{}: Early Stopping".format(j))
                break
        Valid_Score[j] = score
...
W = np.exp(Valid_Score)/np.sum(np.exp(Valid_Score))
# print(W)
...
# te_pred = predict(te_sst,te_t300,te_ua,te_va,W)
...
if UPLOAD:
    ...
    for filename in os.listdir(data_test_dir):
        ...
        prY = predict(te_sst, te_t300, te_ua, te_va, W)
