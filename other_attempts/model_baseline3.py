#!/usr/bin/env python
# coding: utf-8
# max pooling operates on features
# In[11]:
...
#         self.linear = nn.Linear(128, 24)
    ...
    def _define_layers_for_one_feature(self, kernels):
        ...
        pool3 = nn.AdaptiveAvgPool2d((12, 8))  # 对特征而不是时间序列取AvgPooling，从64降到8
        linear = nn.Linear(96, 24)
        tanh = nn.Tanh()
        
        return nn.ModuleList([conv1, pool1, conv2, pool2, batch_norm, lstm, pool3, linear, tanh])
    ...
    def _one_feature_forward(self, inp, layers):
        ...
        conv1, pool1, conv2, pool2, batch_norm, lstm, pool3, linear, tanh = layers
        ...
        # 6: lstm:  [b, 12, 108] --> [b, 12, 64]
        out6 = lstm(out5)[0]
        
        # 7: pool3  [b, 12, 64]     --> [b, 12, 8]
        out7 = pool3(out6)
        
        # 8: flatten + linear + tanh  [b, 12, 8] --> [b, 24]
        out8 = torch.flatten(out7, start_dim=1)
        out8 = linear(out8)
        out8 = tanh(out8)
...
for i in range(1):
    ...
    for step, ((sst, t300, ua, va), label) in enumerate(refine_loader):                
        ...
        loss.backward()
        optimizer.step()
        print('Step: {}, Train Loss: {}'.format(step, loss.detach().cpu().numpy()))
